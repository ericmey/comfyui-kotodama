...
class EvalProtocolTests(unittest.TestCase):
    def test_pair_arms_share_image_seed_and_keep_both_prompts(self):
        ...
        self.assertEqual({item["prompt"] for item in queue}, {"raw", "better", "raw2", "better2"})
        self.assertEqual([item["image_seed"] for item in queue], [900, 900, 901, 901])
        self.assertTrue(all({item["A"], item["B"]} == {"raw", "enhanced"} for item in key))
    ...
    def test_embedded_graph_must_match_prompt_seed_and_pack(self):
        ...
        pack = {
            "loras": [],
            "parameters": {
                "model.unet_name": "model",
                "model.weight_dtype": "default",
                "clip.clip_name": "clip",
                "clip.type": "krea2",
                "clip.device": "default",
                "vae.vae_name": "vae",
